...
-- 둘이었습니다. 지금은 @todo-web@이 더해져 셋이므로 여기도 셋입니다
...
spec = describe "INC-2026-001 회귀" $ do
  it "연결은 WAL 저널 모드를 사용한다" $ do
    modes <- withSystemTempDirectory "todo-store-wal" $ \dir ->
  ...
  it "두 표면이 같은 파일을 동시에 처음 열어도 실패하지 않는다" $
    ...
    withSystemTempDirectory "todo-store-open-race" $ \dir -> do
  ...
  it "세 표면이 같은 파일에 동시에 써도 잠금 오류로 실패하지 않는다" $
    withSystemTempDirectory "todo-store-concurrent" $ \dir -> do
      let path = dir </> "todo.db"
  ...
  it "브라우저 탭이 계속 읽는 동안에도 다른 표면이 쓸 수 있다" $
    -- todo-web은 탭을 열어 두면 연결을 상시 유지합니다. CLI 중심 사용에는 없던
    -- 패턴이며, WAL에서 읽기가 쓰기를 막지 않는다는 전제에 의존합니다.
    withSystemTempDirectory "todo-store-reader" $ \dir -> do
      let path = dir </> "todo.db"
      web <- openStore path
      cli <- openStore path
      api <- openStore path
