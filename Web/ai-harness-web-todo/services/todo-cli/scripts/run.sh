...
usage() {
  cat <<USAGE
사용법: services/$package/scripts/run.sh [명령] [-- 인자...]
...
예시
  ...
  services/$package/scripts/run.sh test
  services/$package/scripts/run.sh run -- --help

의존 패키지는 cabal이 함께 빌드합니다. 예를 들어 todo-cli를 빌드하면
todo-store와 todo-core도 함께 빌드됩니다. 허용된 의존 방향은
...
case "$command_name" in
  ...
  run)
    ...
    if [[ -z "$executable" ]]; then
      ...
      printf '실행 가능한 표면은 todo-cli와 todo-api입니다.\n' >&2
