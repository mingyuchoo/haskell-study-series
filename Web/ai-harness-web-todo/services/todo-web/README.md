...
기본 목록이 `todo-cli`의 `list`와 같습니다. 두 표면 모두 `Todo.Core.Filter.resolveStatusFilter`를 쓰기 때문입니다. `todo-api`는 스크립트가 쓰는 표면이므로 기본값 없이 전체를 반환합니다.
...
| 환경 변수 | 기본값 | 의미 |
|---|---|---|
| `TODO_DB` | `todo.db` | SQLite 파일 경로. `todo-cli` 및 `todo-api`와 공유합니다 |
...
다섯 패키지를 한 번에 다루려면 `../scripts/run.sh`를 사용합니다.
