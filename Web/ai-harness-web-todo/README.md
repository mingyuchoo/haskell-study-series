...
```bash
cabal run todo-cli -- add "보고서 작성" --due 2026-09-01 --tag work --priority high
```
...
```bash
TODO_DB=todo.db TODO_API_PORT=8080 cabal run todo-api
```
...
```bash
TODO_DB=todo.db TODO_WEB_PORT=8081 cabal run todo-web
```
...
| 패키지 | 책임 |
|---|---|
| `services/todo-core` | 도메인 규칙과 유스케이스. 순수 계층이며 `IO`를 모릅니다 |
| `services/todo-store` | SQLite 저장 어댑터 |
| `services/todo-cli` | 명령줄 표면 |
| `services/todo-api` | Servant HTTP 표면 |
| `services/todo-web` | 브라우저 표면. 서버 렌더링, 루프백 전용 |

전체 구조와 의존 규칙은 `ARCHITECTURE.md`에 있습니다.

`services` 아래에는 두 층의 스크립트가 있습니다. 집계 스크립트가 지역 스크립트에 일을 넘기므로 빌드와 테스트 방법의 정의는 한 곳에만 있습니다.

```bash
./services/scripts/run.sh              # 모든 패키지를 빌드하고 테스트
```

```bash
./services/scripts/run.sh web          # 브라우저 표면을 빌드하고 기동
```

```bash
./services/scripts/run.sh web --test --with-api   # 테스트까지 거친 뒤 두 표면을 함께
```
...
```bash
./services/todo-cli/scripts/run.sh test   # 한 패키지만
```

`./services/scripts/run.sh help`과 `list`로 사용 가능한 명령과 앱을 확인할 수 있습니다.
...
도메인 규칙은 services/todo-core에 두고 어댑터에 복제하지 않는다.
