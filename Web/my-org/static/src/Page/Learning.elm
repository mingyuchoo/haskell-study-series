module Page.Learning exposing (view, viewWith, viewWithActivity)
...
viewWith : Mode -> Controls msg -> { a | goals : List GoalView, people : List Person, reviews : List Review, reviewWarnings : List ReviewWarning, events : List Audit } -> Html msg
viewWith =
    viewWithActivity Nothing
...
viewWithActivity activity mode model w =
    div []
        ...
        , if List.isEmpty w.reviews then
            ...
            reviewTable activity w

          else
            div [ class "grid" ] (List.map (reviewCard activity w) w.reviews)
        ]
...
reviewTable activity w =
    ListView.tableView "회고와 학습"
        [ "회고일", "목표", "평가", "회고 요약", "학습", "다음 결정", "경고", "활동 기록" ]
        (List.map
            (\r ->
                tr []
                    [ td [] [ text (String.left 10 r.heldAt) ]
                    , th [ scope "row" ] [ text (goalName w r.goal) ]
                    , td [] [ text (statusName r.evaluation.status) ]
                    , td [] [ text r.note ]
                    , td []
                        (if List.isEmpty r.learnings then
                            [ note "기록된 학습 없음" ]
                            ...
                            List.map (\learning -> p [] [ text learning ]) r.learnings
                        )
                    , td []
                        (if List.isEmpty r.decisions then
                            [ note "기록된 결정 없음" ]
                            ...
                            List.map (\d -> p [] [ text d.text, br [] [], small [] [ text (personName w d.owner ++ " · " ++ (d.deadline |> Maybe.map (String.left 10) |> Maybe.withDefault "기한 미정")) ] ]) r.decisions
                        )
                    , td [] (w.reviewWarnings |> List.filter (.id >> (==) r.id) |> List.concatMap .warnings |> List.map (\warning -> p [ class "tag warn" ] [ text warning ]))
                    , td [] [ activityLink activity r.id ]
                    ]
            )
            w.reviews
        )
...
activityLink activity review =
    activity |> Maybe.map (\go -> button [ type_ "button", class "secondary", onClick (go review) ] [ text "관련 활동 기록 →" ]) |> Maybe.withDefault (text "")
