module StartupSmokeSpec (spec) where
...
spec :: Spec
spec = describe "Process startup and persisted demo lifecycle" $ it "seeds, preserves deletion/replacement, and refuses unrelated data without mutation" $
  withSystemTempDirectory "my-org-startup-" $ \directory -> do
    ...
    let eventFile = directory </> "runs/demo/events.json"
        cleanEnv = filter (\(key, _) -> key `notElem` ["MY_ORG_TEST_DATABASE_URL", "MY_ORG_DEMO", "MY_ORG_EVENT_FILE", "MY_ORG_PORT"]) inherited
        config demo port = (proc binary ["--startup-server", "+RTS", "-N2", "-RTS"])
          { cwd = Just directory
          , create_group = True
          , env = Just (("MY_ORG_EVENT_FILE", eventFile) : ("MY_ORG_PORT", show port) : [("MY_ORG_DEMO", "1") | demo] <> cleanEnv)
          }
        bootstrap continuing = do
          ...
          when continuing $ output `shouldSatisfy` isInfixOf "Continuing the saved demo workspace; your changes are preserved."
          (exitCode, output) `shouldSatisfy` (\(code, logText) -> code == Nothing || code == Just ExitSuccess || "Address already in use" `isInfixOf` logText)
        serve action = do
          (port, socket) <- openFreePort
          close socket
          withFile (directory </> "server.log") WriteMode $ \logHandle ->
            bracket (createProcess (config False port) {std_out = UseHandle logHandle, std_err = UseHandle logHandle}) stop $ \(_, _, _, process) ->
    ...
    serve $ \client -> do
      ...
      _ <- call client "DELETE" ("organizations/" <> identifier) (Just (object ["confirmName" .= field org "name", "expectedVersion" .= field state "version"])) 200
    ...
    serve $ \client -> do
      ...
      _ <- post client "organizations" (object ["id" .= String "new-local-org", "name" .= String "가상의 재생성 조직"]) 201
      void (post client "people" (object ["id" .= String "new-person", "name" .= String "새 가상 인물", "role" .= String "담당자"]) 201)
    ...
    serve $ \client -> void (post client "organizations" (object ["id" .= String "unrelated", "name" .= String "기존 별도 가상 조직"]) 201)
...
runBootstrap directory config = do
  ...
  code <- withFile logFile WriteMode $ \handle ->
    bracket (createProcess config {std_out = UseHandle handle, std_err = UseHandle handle}) stop $ \(_, _, _, process) ->
...
waitReady client process attempts = do
  ...
  case running of
    ...
    Nothing -> do
      response <- try (request client "GET" "dashboard" Nothing) :: IO (Either HttpException (Int, Value))
      case response of
        Right (200, _) -> pure ()
