...
spec = describe "agent role derivation, diagnostics and design commands" $ do
  it "derives one role per stored workflow using references before text" $ do
    ...
    map agentId drafts
      `shouldBe` [ "agent-demo-w-inquiry"
                 , "agent-demo-w-refund"
                 , "agent-demo-w-proposal"
                 , "agent-demo-w-contract"
                 ]
    ( agentName inquiry
      , agentPermissionLevel inquiry
      , agentTools inquiry
      , agentHandoffTo inquiry
      )
    ...
    ( agentName contract
      , agentPermissionLevel contract
      , agentTools contract
      , agentApprovalBy contract
      , agentStatus contract
      )
      `shouldBe` ("파트너십 계약 검토 담당", L0Read, [], Nothing, Unknown)
    map agentSourceWorkflow drafts
      `shouldBe` map Just ["demo-w-inquiry", "demo-w-refund", "demo-w-proposal", "demo-w-contract"]
  it "falls back to active member names and workflow names mentioned in free text" $ do
    ...
    let intake =
        ...
        inactive =
          deriveAgents
            st {stateDiscovery = survey, stateInactivePeople = Set.singleton (UserId "demo-success")}
    ( agentHandoffTo (byId "agent-intake" drafts)
      , agentApprovalBy (byId "agent-intake" drafts)
      )
      `shouldBe` (["agent-refund"], Just (ApprovalPerson (UserId "demo-success")))
    agentApprovalBy (byId "agent-refund" drafts)
      `shouldBe` Just (ApprovalPerson (UserId "demo-ceo"))
  ...
  it "diagnoses the demo drafts without inventing confirmed facts" $ do
    ...
    map agentDiagnosticSeverity diagnostics
      `shouldSatisfy` (\xs -> and (zipWith (<=) xs (drop 1 xs)))
  it "flags unresolved handoffs, approvers, levels, tools and stale sources" $ do
    ...
    let inactive = st {stateInactivePeople = Set.singleton (UserId "demo-sales")}
        agents =
          ...
          , (agent "b")
              { agentPermissionLevel = L2External
              , agentApprovalBy = Just (ApprovalPerson (UserId "nobody"))
              }
          , (agent "c")
              { agentPermissionLevel = L2External
              , agentApprovalBy = Just (ApprovalPerson (UserId "demo-sales"))
              }
          , (agent "d")
              { agentPermissionLevel = L2External
              , agentApprovalBy = Just (ApprovalPermission Hiring)
              }
        ...
        noHiring =
          inactive
            { stateAuthorities = Map.map (revokePermission Hiring) (stateAuthorities inactive)
            }
  ...
  it "validates the saved design against the organization and version" $ do
    ...
    let version = stateLastSeq st
        ...
        valid =
          [ (agent "review") {agentHandoffTo = ["execute"]}
          , (agent "execute")
              { agentPermissionLevel = L2External
              , agentApprovalBy = Just (ApprovalPerson (UserId "demo-ceo"))
              }
          ]
    ...
    save [(agent "a") {agentStatus = Confirmed, agentEvidence = ""}] version
    ...
    let raw =
        ...
        expected =
          StoredEvent
            ...
            Nothing
            ( DiscoverySaved
                emptyDiscovery {discoveryScope = "s", discoveryWorkflows = [emptyWorkflow "w" "n"]}
            )
    decodeStoredEvent raw `shouldBe` Right expected
    -- Re-encoding a workflow without references adds no new keys.
    BL.unpack (encodeStoredEvent expected)
      `shouldSatisfy` (not . T.isInfixOf "handoffWorkflows" . T.pack)
