...
spec = describe "typed queries" $ do
  it "preserves empty legacy organization and dashboard results" $ do
    ...
    executeQuery now registry (OrganizationQuery SoleOrganization OrganizationResource)
      `shouldBe` Right (OrganizationResult Nothing)
    ...
    executeQuery now registry (OrganizationSummaryQuery (OrgId "missing"))
      `shouldBe` Left (OrganizationNotFound (OrgId "missing"))
  it
    "projects deterministic evaluations, counts and timeline ordering using the supplied time" $ do
    ...
    case executeQuery (addUTCTime 86400 now) registry query of
      Right (DashboardResult dashboard) -> do
        map (evaluationEvaluatedAt . viewEvaluation) (dashboardGoals dashboard)
          `shouldBe` replicate 7 (addUTCTime 86400 now)
        ...
        executeQuery now registry (OrganizationQuery SoleOrganization EventsResource)
          `shouldBe` Right (EventsResult events)
      value -> expectationFailure (show value)
  it
    "requires explicit scope with multiple organizations and exposes only the recreated epoch" $ do
    let a = OrgId "a"
        b = OrgId "b"
        event oid payload = OrganizationScoped oid payload
        events =
          zipWith
            (\n payload -> StoredEvent n now Nothing payload)
            [1 ..]
    ...
    executeQuery now registry (OrganizationQuery (SelectedOrganization a) EventsResource)
      `shouldBe` Right
        ( EventsResult
            [StoredEvent 4 now Nothing (OrganizationCreated (Organization a "New A" now))]
        )
  it "maps only known HTTP resources to typed queries" $ do
    readRoute ["api", "organizations", "a", "goals"]
      `shouldBe` Just (OrganizationQuery (SelectedOrganization (OrgId "a")) GoalsResource)
    readRoute ["api", "dashboard"]
      `shouldBe` Just (OrganizationQuery SoleOrganization DashboardResource)
