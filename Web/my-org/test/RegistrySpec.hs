module RegistrySpec (registrySpec) where
...
registrySpec = describe "여러 조직 레지스트리" $ do
  it "A와 B 생성·이름 변경·A 삭제는 B 상태와 버전에 영향이 없다" $ temporary $ \path ->
    bracket (openFileStore path) closeStore $ \store -> do
      ...
      a <- stateOf store aid
      b <- stateOf store bid
      ok $ runOrganizationCommand store aid Nothing (RenameOrganization aid "새 A" (stateLastSeq a))
      stateOf store bid `shouldReturn` b
      a2 <- stateOf store aid
      fmap organizationName (stateOrganization a2) `shouldBe` Just "새 A"
      fmap organizationCreatedAt (stateOrganization a2) `shouldBe` fmap organizationCreatedAt (stateOrganization a)
      ok $ runOrganizationCommand store aid Nothing (DeleteOrganization aid "새 A" (stateLastSeq a2))
      stateOf store bid `shouldReturn` b
      registry <- readRegistry store
      map (fmap organizationId . stateOrganization) (activeOrganizations registry) `shouldBe` [Just bid]
  it "B 변경은 A 확인 버전을 만료시키지 않지만 같은 A 변경은 충돌한다" $ temporary $ \path ->
    bracket (openFileStore path) closeStore $ \store -> do
      createTwo store
      a <- stateOf store aid
      ok $ runOrganizationCommand store bid Nothing (AddPerson person)
      ok $ runOrganizationCommand store aid Nothing (RenameOrganization aid "A 수정" (stateLastSeq a))
      runOrganizationCommand store aid Nothing (RenameOrganization aid "오래된 수정" (stateLastSeq a)) >>= (`shouldSatisfy` isLeft)
  it "같은 사람·목표·회고 ID가 조직별로 독립적인 결과와 권한을 가진다" $ temporary $ \path ->
    bracket (openFileStore path) closeStore $ \store -> do
      ...
      forM_ [aid,bid] $ \oid -> do
        ...
        ok $ runOrganizationCommand store oid Nothing (ReportResult gid (if oid == aid then 20 else 80) uid "실측")
        ok $ runOrganizationCommand store oid Nothing (HoldReview (ReviewId "same-review") gid [Learning "학습"] [] "회고")
      a <- stateOf store aid
      b <- stateOf store bid
      ...
      ok $ runOrganizationCommand store aid Nothing (GrantAuthority (Authority uid 500 True False Set.empty))
      stateOf store bid `shouldReturn` b
  it "다른 조직의 actor·owner·reporter·decisionOwner·reportsTo·parent를 거부한다" $ temporary $ \path ->
    bracket (openFileStore path) closeStore $ \store -> do
      createTwo store
      ok $ runOrganizationCommand store aid Nothing (AddPerson (Person (UserId "a-only") "A 전용" "담당자" Nothing))
      ok $ runOrganizationCommand store aid Nothing (CreateGoal (goal aid) {goalId = GoalId "a-only-goal"})
      ok $ runOrganizationCommand store bid Nothing (AddPerson person)
      ...
      runOrganizationCommand store bid (Just other) (EvaluateGoal gid) >>= (`shouldSatisfy` isLeft)
      forM_ [ AssignOwner gid other
  ...
  it "동시 이름 변경과 삭제에서 같은 버전은 한 번만 성공한다" $ temporary $ \path ->
    bracket (openFileStore path) closeStore $ \store -> do
      createTwo store
      a <- stateOf store aid
      boxes <- forM [RenameOrganization aid "변경" (stateLastSeq a), DeleteOrganization aid "A 조직" (stateLastSeq a)] $ \command -> do
        box <- newEmptyMVar
  ...
  it "저장 실패 시 모든 조직과 파일이 그대로 남는다" $ temporary $ \path ->
    bracket (openFileStore path) closeStore $ \store -> do
      ...
      runOrganizationCommand store aid Nothing (RenameOrganization aid "실패" (stateLastSeq a)) `shouldReturn` Left StorageFailure
  ...
  it "다른 조직과 함께 데모 생성·중복거절·삭제후재시드가 가능하다" $ temporary $ \path ->
    bracket (openFileStore path) closeStore $ \store -> do
      ...
      ok $ runOrganizationCommand store demoOrganizationId Nothing (DeleteOrganization demoOrganizationId (organizationName org) (stateLastSeq demo))
      ok $ seedDemo store
      stateOf store bid `shouldReturn` b
    ...
    case demoEvents now of
      ...
      Right marked -> forM_ [init marked, marked] $ \legacy -> temporary $ \path -> do
        ...
        bracket (openFileStore path) closeStore $ \store -> do
          ...
          registry <- readRegistry store
          isDemoEpoch (Map.findWithDefault [] demoOrganizationId (registryEvents registry)) `shouldBe` True
        BL.readFile path `shouldReturn` bytes
  it "scoped 이벤트는 구형 로그의 소속 커서를 바꾸지 않는다" $ do
    let stream = zipWith (\n e -> StoredEvent n now Nothing e) [1..]
    ...
    case replayRegistry stream of
      ...
      Right registry -> do
        ...
        fmap (Map.keys . statePeople) (organizationState registry bid) `shouldBe` Right [UserId "b-only"]
        registryLastSeq registry `shouldBe` 4
  it "구형 삭제/동일ID 재생성은 과거 데이터를 섞지 않는다" $ do
    let stream = zipWith (\n e -> StoredEvent n now Nothing e) [1..]
          [ OrganizationCreated (Organization aid "A 조직" now), PersonAdded person
          , OrganizationDeleted aid, OrganizationCreated (Organization aid "새 A" now)]
    case replayRegistry stream of
      Left err -> expectationFailure (show err)
      Right registry -> do
        fmap (Map.null . statePeople) (organizationState registry aid) `shouldBe` Right True
        map storedSeq (currentEpoch (Map.findWithDefault [] aid (registryEvents registry))) `shouldBe` [4]
  it "중첩 scope와 payload 조직 불일치 로그를 거부한다" $ do
    let bad e = replayRegistry [StoredEvent 1 now Nothing e]
    bad (OrganizationScoped aid (OrganizationScoped aid (OrganizationCreated (Organization aid "A" now)))) `shouldSatisfy` isLeft
    bad (OrganizationScoped aid (OrganizationCreated (Organization bid "B" now))) `shouldSatisfy` isLeft
...
stateOf store oid = readRegistry store >>= either (\err -> expectationFailure (show err) >> pure emptyState) pure . (`organizationState` oid)
createTwo :: Store -> IO ()
createTwo store = do
...
goal :: OrgId -> Goal
goal oid = Goal gid oid "테스트 목표" (Metric (MetricId "metric") "지표" "점" HigherIsBetter) 0 100 now (addUTCTime (365*86400) now) Nothing Set.empty 0
forM_ :: [a] -> (a -> IO b) -> IO ()
