...
field (Object fields) key =
  maybe
    (error ("Missing JSON field: " <> show key))
    id
...
goal identifier organization =
  object
    [ "id" .= identifier
    , "organization" .= organization
    , "description" .= ("매출 성장" :: Text)
    , "metric"
        .= object
          [ "id" .= ("sales" :: Text)
          , "name" .= ("매출" :: Text)
          , "unit" .= ("KRW" :: Text)
          , "direction" .= ("HigherIsBetter" :: Text)
          ]
    , "baseline" .= (0 :: Int)
    , "target" .= (100 :: Int)
    , "startsAt" .= ("2026-01-01T00:00:00Z" :: Text)
    , "deadline" .= ("2027-01-01T00:00:00Z" :: Text)
    , "requiredPermissions" .= ([] :: [Text])
    , "requiredBudget" .= (0 :: Int)
    ]

authority :: Text -> Int -> Bool -> Value
authority owner budget pricing =
  object
    [ "owner" .= owner
    , "budgetLimit" .= budget
    , "canHire" .= False
    , "canChangePrice" .= pricing
    , "canApprove" .= ([] :: [Text])
    ]
