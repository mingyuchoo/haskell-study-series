...
module ContractSpec (spec, fixtureTime, normalizeDashboard, withFixtureServer) where

import Control.Exception (bracket)
import MyOrg.Serialization.JSON (Wire, toWire, eitherDecodeWire)
...
import MyOrg.Types (OrgId(..), UserId(..), Permission(..))
import MyOrg.Store (openFileStore, closeStore)
import Network.Wai.Handler.Warp (testWithApplication)
import SmokeSupport (Client, withClient, get, field, items)
...
spec = describe "persisted event and Elm HTTP contracts" $ do
  it "decodes historical legacy and scoped logs and preserves their exact JSON shapes" $ do
    ...
    let scoped = map (\event -> event {storedEvent = OrganizationScoped demoOrganizationId (storedEvent event)}) baseline
    mapM_ (\(name, expected) -> do
  ...
  it "preserves lifecycle and revoked-permission event tags absent from the demo" $ do
    let raw = "[{\"seq\":1,\"at\":\"2000-01-01T00:00:00Z\",\"event\":{\"tag\":\"OrganizationRenamed\",\"contents\":[\"org\",\"Renamed\"]}},{\"seq\":2,\"at\":\"2000-01-01T00:00:00Z\",\"actor\":\"owner\",\"event\":{\"tag\":\"AuthorityRevoked\",\"contents\":[\"owner\",\"Pricing\"]}},{\"seq\":3,\"at\":\"2000-01-01T00:00:00Z\",\"event\":{\"tag\":\"OrganizationDeleted\",\"contents\":\"org\"}}]"
        expected = [ StoredEvent 1 fixtureTime Nothing (OrganizationRenamed (OrgId "org") "Renamed")
                   , StoredEvent 2 fixtureTime (Just (UserId "owner")) (AuthorityRevoked (UserId "owner") Pricing)
  ...
  it "serves the fixed dashboard contract from either historical storage format" $ do
    ...
    mapM_ (\name -> withFixtureServer name $ \client -> do
      ...
      mapM_ (\entry -> case field (field entry "evaluation") "evaluatedAt" of
        String value -> (eitherDecode (encode value) :: Either String UTCTime) `shouldSatisfy` either (const False) (const True)
