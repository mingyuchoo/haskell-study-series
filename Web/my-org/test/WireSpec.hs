...
spec = describe "explicit boundary codecs" $ do
  it "preserves nullary and three-argument sum contracts" $ do
    ...
    contract (MissingPermissions (GoalId "g") (UserId "u") (Set.fromList [Pricing, Hiring]))
  ...
  it "rejects unknown enum/constructor tags and wrong tuple arity" $ do
    ...
    (eitherDecodeWire "{\"tag\":\"UnknownEvent\"}" :: Either String OrganizationEvent) `shouldSatisfy` isLeft
    (eitherDecodeWire "{\"tag\":\"OwnerAssigned\",\"contents\":[\"g\",\"u\",\"extra\"]}" :: Either String OrganizationEvent) `shouldSatisfy` isLeft
    (eitherDecodeWire "{\"tag\":\"OwnerAssigned\",\"contents\":[\"g\"]}" :: Either String OrganizationEvent) `shouldSatisfy` isLeft
  it "accepts absent/null optional values, omits them on write, and rejects mistypes" $ do
    ...
    eitherDecodeWire "{\"id\":\"u\",\"name\":\"Name\",\"role\":\"Role\",\"reportsTo\":null}" `shouldBe` Right person
    (eitherDecodeWire "{\"id\":\"u\",\"name\":\"Name\",\"role\":\"Role\",\"reportsTo\":42}" :: Either String Person) `shouldSatisfy` isLeft
    (eitherDecodeWire "{\"id\":\"u\",\"name\":\"Name\"}" :: Either String Person) `shouldSatisfy` isLeft
  it "keeps user maps as JSON objects with literal Unicode keys" $
    contract (Map.fromList [(UserId "owner", 0.75 :: Double), (UserId "한글", 0.25)])
      "{\"owner\":0.75,\"한글\":0.25}"
  it "renders structured draft errors as the existing diagnostic text projection" $ do
    let diagnostic = Diagnostic "O009" Error "goal" (InvalidDraft (InvalidInput "텍스트는 1~10000자여야 합니다.")) []
        raw = BL.fromStrict (TE.encodeUtf8 "{\"code\":\"O009\",\"severity\":\"Error\",\"subject\":\"goal\",\"message\":\"텍스트는 1~10000자여야 합니다.\",\"details\":[]}")
