module DemoSmokeSpec (spec) where
...
spec = describe "HTTP demo initialization" $ it "atomically seeds once under four concurrent requests" $ withFreshServer $ \client -> do
  responses <- mapConcurrently (\_ -> request client "POST" "demo" (Just empty)) [1 .. 4 :: Int]
  ...
  sort (nub (map (\entry -> field (field entry "evaluation") "status") active)) `shouldBe`
    sort (map String ["NoData", "OnTrack", "AtRisk", "OffTrack", "Achieved"])
  length goals - length active `shouldSatisfy` (>= 2)
  goals `shouldSatisfy` any (\entry -> field entry "owner" == Null)
  map (`field` "code") (items (field (field state "compiler") "diagnostics")) `shouldHave` map String ["O001", "O017", "O031", "O040"]
  map (`field` "kind") (items (field (field state "graph") "edges")) `shouldHave` map String ["Owns", "DependsOn", "Controls", "Measures"]
  let reviews = items (field state "reviews")
  length reviews `shouldSatisfy` (>= 2)
  reviews `shouldSatisfy` any (\review -> not (null (items (field review "learnings"))) && not (null (items (field review "decisions"))))
