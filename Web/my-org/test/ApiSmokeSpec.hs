...
spec = describe "HTTP API lifecycle" $ it "preserves validation, evaluation, reviews, events and authority revocation" $ withFreshServer $ \client -> do
          ...
          Object
            ( KM.union
                ( KM.fromList
                    [ ("baseline", Number 100)
                    , ("target", Number 200)
                    , ("requiredBudget", Number 100)
                    , ("requiredPermissions", toArray [String "Pricing"])
                    ]
                )
                fields
            )
  ...
  send
    ...
    ( object
        [ "id" .= String "owner"
        , "name" .= String "<img src=x onerror=alert(1)>"
        , "role" .= String "영업"
        ]
    )
    201
  ...
  send
    "goals/revenue/results"
    ( object
        [ "value" .= Number 200
        , "reportedBy" .= String "owner"
        , "note" .= String "달성"
        , "actor" .= String "owner"
        ]
    )
  ...
  send
    "reviews"
    (object ["id" .= String "review1", "goal" .= String "revenue", "note" .= String "학습 없는 회고"])
    201
  send
    "reviews"
    ( object
        ...
        , "goal" .= String "revenue"
        , "note" .= String "잘못된 결정"
        , "decisions" .= [object ["text" .= String "실험", "owner" .= String "missing"]]
        ]
    )
    404
  send
    ...
    ( object
        ...
        , "goal" .= String "revenue"
        , "note" .= String "다음 실험"
        , "learnings" .= [object ["text" .= String "가격 권한 위임이 효과적"]]
        , "decisions"
            .= [ object
                   [ "text" .= String "가격 실험"
                   , "owner" .= String "owner"
                   , "deadline" .= String "2027-02-01T00:00:00Z"
                   ]
               ]
        ]
    )
  ...
  field (field (first (items (field state "goals"))) "evaluation") "status"
    `shouldBe` String "Achieved"
  length (items (field state "reviews")) `shouldBe` 2
  map (`field` "code") (items (field (field state "compiler") "diagnostics"))
    `shouldContain` [String "O040"]
  field (first (items (field state "people"))) "name"
    `shouldBe` String "<img src=x onerror=alert(1)>"
  mapM_
    (get client)
    ["organization", "people", "goals", "compiler", "graph", "events", "reviews"]
  events <- items <$> get client "events"
  map (number . (`field` "seq")) events `shouldBe` [1 .. length events]
  events
    `shouldSatisfy` any (\case Object fields -> KM.lookup "actor" fields == Just (String "owner"); _ -> False)
  ...
  send
    "goals/revenue/results"
    (object ["value" .= Number 200, "reportedBy" .= String "owner", "note" .= String "권한 회수 후"])
    409
  send "people/owner/authority" auth 201
