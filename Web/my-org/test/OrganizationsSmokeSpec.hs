module OrganizationsSmokeSpec (spec) where
...
spec = describe "HTTP multiple organizations" $ it "isolates scoped CRUD, actors, references, revisions and demo data" $ withFreshServer $ \client -> do
  ...
  forM_ ["a", "b"] $ \oid -> do
    ...
    send (org oid "goals/same-goal/results") (object ["value" .= (if oid == "a" then 20 else 80 :: Int), "reportedBy" .= String "same", "note" .= String "결과"]) 201
    send (org oid "evaluations") (object ["goal" .= String "same-goal"]) 201
    send (org oid "reviews") (object ["id" .= String "same-review", "goal" .= String "same-goal", "note" .= String "회고", "learnings" .= [object ["text" .= (T.pack oid <> " 학습")]]]) 201
  ...
  send (org "a" "goals/same-goal/results") (object ["value" .= Number 2, "reportedBy" .= String "b-only", "note" .= String "결과"]) 404
  send (org "a" "reviews") (object ["id" .= String "invalid", "goal" .= String "same-goal", "note" .= String "회고", "decisions" .= [object ["text" .= String "결정", "owner" .= String "b-only"]]]) 404
  b <- stable <$> get client (org "b" "dashboard")
  currentA <- get client (org "a" "")
  _ <- call client "DELETE" (org "a" "") (Just (object ["confirmName" .= String "A 새 이름", "expectedVersion" .= field currentA "version"])) 200
  ...
  _ <- call client "DELETE" (org "a" "") (Just (object ["confirmName" .= String "A 재생성", "expectedVersion" .= field currentA "version"])) 409
  send "demo" empty 201
  (stable <$> get client (org "b" "dashboard")) `shouldReturn` b
  send "demo" empty 409
