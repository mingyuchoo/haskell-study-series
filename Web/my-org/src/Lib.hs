...
someFunc = do
  ...
  let chosenFile = if demo then "runs/demo/events.json" else file
      ...
      open = if demo then openFileStore chosenFile else maybe (openFileStore chosenFile) (openPostgresStore . BS.pack) database
  putStrLn ("My Org: http://127.0.0.1:" <> show chosenPort <> " (local, unauthenticated MVP)")
  bracket open closeStore $ \store -> do
    if demo then do
      events <- readAudit store
      registry <- readRegistry store
      if null events then do
        result <- seedDemo store
        either (const (ioError (userError "Demo initialization failed; existing data was not changed"))) (const (pure ())) result
      else if any isDemoStore (Map.elems (registryEvents registry))
        then putStrLn "Continuing the saved demo workspace; your changes are preserved."
        else ioError (userError "The demo path contains another workspace; refusing to change it.")
    else pure ()
