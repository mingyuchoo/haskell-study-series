...
module MyOrg.Infrastructure.FileStore (openFilePersistence) where
...
openFilePersistence path = do
  ...
  (do
    ...
    let persist xs = bracketOnError (openBinaryTempFile dir ".my-org-events.tmp") (\(tmp,h) -> hClose h >> removeFile tmp) $ \(tmp,h) -> do
          ...
          hClose h
          renameFile tmp path
    pure (Persistence events persist (removeDirectory lock))) `onException` removeDirectory lock

decodeEvents :: BL.ByteString -> IO [StoredEvent]
decodeEvents bytes = either (ioError . userError . ("Corrupt event file: " <>)) pure (eitherDecodeWire bytes)
