...
module MyOrg.Infrastructure.PostgresStore (openPostgresPersistence) where
...
openPostgresPersistence connectionString = do
  ...
  (do
    ...
    events <- mapM (\(Only payload) -> either (ioError . userError) pure (eitherDecodeWire (BL.fromStrict (TE.encodeUtf8 payload)))) rows
    let persist xs = withTransaction connection $ do
          ...
          mapM_ (\event -> void $ execute connection "INSERT INTO my_org_events(sequence,payload) VALUES (?,?)" (storedSeq event, TE.decodeUtf8 (BL.toStrict (encodeWire event)))) (drop count xs)
    pure (Persistence events persist (close connection))) `onException` close connection
