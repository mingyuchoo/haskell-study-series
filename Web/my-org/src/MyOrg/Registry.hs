...
module MyOrg.Registry
  ( Registry(..), replayRegistry, activeOrganizations, organizationState
...
replayRegistry events = fst <$> foldM step (Registry Map.empty Map.empty 0, Nothing) events
 where
  step (registry, legacy) stored = do
    unless (storedSeq stored == registryLastSeq registry + 1) (Left (InvalidInput "이벤트의 전역 순번이 연속되지 않습니다."))
