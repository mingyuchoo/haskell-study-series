...
presentAnalysis Analysis {..} = AnalysisView analysisGoal analysisOwner analysisCoverage analysisStatus
  (map presentHolder analysisResources) (renderCause analysisCause) (map presentRecommendation analysisRecommendations)
  where
    presentHolder ResourceHolder {..} = ResourceHolderView (renderResource holderResource) holderRequired holderOwnerHas holderControlledBy
...
renderCause = \case
  ...
  ResourcesControlled uid -> unUserId uid <> "이(가) 목표 달성에 필요한 자원을 모두 통제하고 있습니다. "
    <> "구조적 병목은 발견되지 않았으며, 실행 자체를 점검해야 합니다."
  InsufficientAuthority uid description share -> unUserId uid <> " owns " <> description
    <> ". However, " <> unUserId uid <> " controls only " <> pct share
...
renderAnalysis analysis =
  ...
  in T.unlines $
    ...
    ++ [resource h <> " authority -> " <> (if null (controlledBy h) then "(nobody)" else T.intercalate ", " (map unUserId (controlledBy h))) | h <- resources a]
    ++ ["", "Recommendation:"]
    ++ zipWith (\i r -> T.pack (show (i :: Int)) <> ". " <> renderRec r) [1..] (recommendations a)
  where
    renderRec = \case
