...
module MyOrg.Demo (demoOrganizationId, demoCommands, demoEvents, isDemoEpoch, isDemoStore) where
...
demoCommands now =
  ...
  , CreateGoal (goal "company" "지속 가능한 전사 성장" "growth-index" "전사 성장 지수" "점" HigherIsBetter 0 100 Nothing [BudgetApproval] 1000000)
  , CreateGoal (child "revenue" "엔터프라이즈 매출 확대" "revenue" "엔터프라이즈 매출" "억원" HigherIsBetter 30 50 [Pricing, Marketing] 0)
  , CreateGoal (child "customers" "새로운 고객과의 첫 만남" "customers" "신규 고객 수" "명" HigherIsBetter 1000 5000 [Marketing] 0)
  , CreateGoal (child "reliability" "고객이 믿고 쓰는 안정적인 서비스" "incidents" "월간 장애 건수" "건" LowerIsBetter 10 0 [Infrastructure] 0)
  , CreateGoal (child "retention" "고객이 계속 찾는 서비스" "churn" "고객 이탈률" "%" LowerIsBetter 8 3 [] 0)
  , CreateGoal (child "launch" "신제품 출시로 고객 가치 전달" "launch-progress" "제품 출시 준비율" "%" HigherIsBetter 0 100 [ProductLaunch, Hiring] 30000000)
  , CreateGoal (child "partners" "새로운 파트너십 확장" "partnerships" "신규 파트너 계약" "건" HigherIsBetter 0 10 [Contracting] 0)
  ]
  <> [AssignOwner (gid g) u | (g,u) <- [("company",ceo),("revenue",sales),("customers",sales),("reliability",infra),("retention",success),("launch",productLead)]]
  <> [ActivateGoal (gid g) | g <- ["company","revenue","customers","reliability","retention"]]
  <> concat [samples "revenue" sales [34,38,42], samples "customers" sales [1300,1800,2280], samples "reliability" infra [10,9,9], samples "retention" success [7,5,3]]
  <> [EvaluateGoal (gid g) | g <- ["company","revenue","customers","reliability","retention"]]
  <> [ HoldReview (ReviewId "demo-review-retention") (gid "retention")
     ...
     , HoldReview (ReviewId "demo-review-customers") (gid "customers") [] []
 ...
 where
  ...
  goal ident description mid metricName unit direction baseline target parent ps budget =
    Goal (gid ident) demoOrganizationId description (Metric (MetricId ("demo-" <> mid)) metricName unit direction)
      baseline target (addUTCTime (-50 * 86400) now) (addUTCTime (50 * 86400) now) parent (Set.fromList ps) budget
  child ident description mid metricName unit direction baseline target =
    goal ident description mid metricName unit direction baseline target (Just (gid "company"))
  samples ident reporter values =
    [ReportResult (gid ident) value reporter "체험용 연속 측정 샘플입니다. 실제 과거 측정이 아니며 가져온 시각으로 기록했습니다." | value <- values]

demoEvents :: UTCTime -> Either OrganizationError [StoredEvent]
demoEvents now = do
  (_, events) <- foldM step (emptyState, []) (demoCommands now)
  pure (events <> [StoredEvent (length events + 1) now (Just (UserId "demo-ceo")) (DemoSeeded demoOrganizationId)])
 where
  step (st, events) command = do
    changes <- executeCommand now st command
    let actor = if Map.member (UserId "demo-ceo") (statePeople st) then Just (UserId "demo-ceo") else Nothing
...
marked = any (\event -> case storedEvent event of DemoSeeded oid -> oid == demoOrganizationId; _ -> False)
