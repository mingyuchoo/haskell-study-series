...
resourceHolderViewCodec = Codec encode decode
  where
    encode Analysis.ResourceHolderView {..} = record
...
analysisViewCodec = Codec encode decode
  where
    encode Analysis.AnalysisView {..} = record
      ...
      , ("recommendations", Just (encodeValue (listCodec recommendationViewCodec) recommendations))
      ]
...
recommendationViewCodec = Codec encode decode
  where
    encode = \case
      Analysis.IncreaseAuthorityView a b -> tagged "IncreaseOwnerAuthority" (Just (encodeValue (listCodec valueCodec) [encodeValue userIdCodec a, encodeValue (listCodec textCodec) b]))
    ...
    decode = withObject "Analysis.RecommendationView" $ \obj -> do
      ...
      case tag of
        "IncreaseOwnerAuthority" -> do
          ...
          case values of
            [a, b] -> Analysis.IncreaseAuthorityView <$> decodeValue userIdCodec a <*> decodeValue (listCodec textCodec) b
