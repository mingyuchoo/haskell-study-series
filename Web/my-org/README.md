...
Haskell API와 Elm 웹 UI로 현재 조직의 역할·책임·권한과 업무 흐름을 정리하는 로컬 웹 MVP입니다. 입력한 업무에서 에이전트 역할 초안을 규칙 기반으로 도출하고, 사람이 권한 등급·승인 주체·인계 대상을 검토해 설계안으로 저장한 뒤 인계 구조를 확인하고 정의 파일로 내보냅니다. 기존 목표 → 최종 책임자 → 결정 권한 → 결과 → 평가 → 학습 기능은 조직 운영을 지원합니다.
...
업무 흐름에는 업무 이름, 담당 역할, 시작 조건, 입력 정보, 사용 도구, 산출물, 인계 대상, 사람 승인 조건을 기록합니다. 텍스트와 별도로 **참조 연결**을 둘 수 있습니다. 담당 구성원(`rolePerson`), 승인 구성원(`approvalPerson`), 승인에 필요한 결정 권한(`approvalPermission`), 인계 대상 업무(`handoffWorkflows`)는 현재 조직의 활성 구성원과 같은 문서의 다른 업무만 가리킬 수 있으며, 없는 구성원은 404, 자기 자신이나 없는 업무로의 인계는 400입니다. 참조 필드는 비어 있으면 JSON에 나타나지 않으므로 이전 저장 기록은 그대로 읽힙니다. 실제 LLM 호출이나 에이전트 실행, 도구 접근 권한 발급은 하지 않습니다. 비어 있는 업무 정보는 미확인으로 표시합니다. 입력 근거를 검토하고 검토 의견을 저장할 수 있으며 현황 본문을 바꾸면 검토 상태가 다시 `pending`이 됩니다.
...
에이전트 역할은 `Domain.Agent`의 도메인 타입입니다. ID, 이름, 도출 근거 업무, 담당 업무, 입력, 산출물, 허용 도구 후보, 권한 등급(`L0` 읽기, `L1` 작업 공간 쓰기, `L2` 외부 영향, `L3` 금지), 사람 승인 주체(구성원 또는 결정 권한 보유자), 인계 대상 역할 ID, 정보 구분과 근거를 가집니다. 설계를 저장해도 도구 접근이나 조직 권한은 부여되지 않습니다.

`Domain.AgentDraft.deriveAgents`가 저장된 업무 흐름마다 하나의 역할 후보를 결정적으로 만듭니다. 규칙은 다음과 같습니다.

- 이름은 담당 역할 텍스트이고, 비어 있으면 "업무 이름 담당"입니다.
- 사람 승인 조건이 있으면 `L2`, 도구가 있으면 `L1`, 그 외 `L0`입니다.
- 승인 주체는 참조 필드를 우선 사용하고, 없으면 승인 텍스트에 이름이 포함된 활성 구성원을 찾습니다.
- 인계 대상은 참조 필드를 우선 사용하고, 없으면 인계 텍스트에 업무 이름이나 담당 역할이 포함된 다른 업무를 찾습니다.
- 도구는 쉼표나 줄바꿈으로 나누고, 상태와 근거는 업무 흐름 값을 그대로 옮깁니다. 비어 있는 정보는 채우지 않습니다.

`diagnoseAgents`는 초안과 저장된 설계에 같은 규칙을 적용합니다.

| 코드 | 심각도 | 의미 |
| --- | --- | --- |
| A001 | 오류 | 인계 대상 역할이 설계에 없음 |
| A002 | 오류 | 승인 주체가 구성원 명단에 없거나 비활성 |
| A003 | 경고 | 승인에 필요한 결정 권한을 가진 활성 구성원이 없음 |
| A004 | 경고 | `L2` 외부 영향 행위인데 사람 승인 주체가 없음 |
| A005 | 오류 | `L3` 금지 행위를 에이전트에 배정 |
| A006 | 정보 | `L1` 이상인데 사용 도구가 미확인 |
| A007 | 경고 | 도출 근거 업무가 더 이상 없음 |
| A008 | 경고 | 근거가 확인되지 않은 역할 |
| A009 | 정보 | 산출물의 인계 대상이 미확인 |

- `GET /api/organizations/:id/agents`: `{version, agents, drafts, diagnostics, draftDiagnostics}`. `agents`는 저장된 설계, `drafts`는 현재 업무 흐름에서 도출한 초안이며 진단은 각각 계산합니다.
- `POST /api/organizations/:id/agents`: `{expectedVersion, agents}` 목록 전체를 저장합니다. ID는 식별자 규칙을 따르고 고유해야 하며, 이름은 비어 있을 수 없고, 인계 대상은 같은 목록의 다른 역할이어야 합니다. 승인 구성원은 활성 구성원이어야 하고(없으면 404), 확인된 사실에는 근거가 필요합니다. 최대 200개 역할, 역할당 도구 50개입니다. 버전 충돌은 409입니다.
- `GET /api/organizations/:id/agents/export`: `text/markdown` 첨부 파일. 저장된 설계가 있으면 설계를, 없으면 초안을 `.claude/agents/<id>.md` 형식의 YAML 프런트매터와 Markdown 본문으로 내보냅니다. 실행 설정이 아니라 설계 기록입니다.

화면에서는 **에이전트 초안**에서 초안과 진단을 보고 설계안으로 가져와 등급, 승인 주체, 도구, 인계 대상을 편집한 뒤 저장합니다. 저장 전 입력은 화면 이동 시 유지되고 버전 충돌은 재적용을 요구합니다. **에이전트 구조**는 저장된 설계(없으면 초안)의 인계 구조를 층으로 나눈 SVG와 관계 목록으로 보여 줍니다. 조직 진단의 검토 의견과 상태는 에이전트 초안 화면 아래에서 계속 저장합니다.
...
| `App.Discovery`, `App.Agents` | 조직별 현황 문서와 에이전트 설계안의 저장본·초안·버전 충돌 관리 |
| `Domain`, `Domain.Permission` | 화면에서 쓰는 데이터와 권한 종류 |
| `Domain.Discovery`, `Domain.Agent` | 현황 문서의 참조 필드 편집, 에이전트 역할 편집과 순수 검증 |
| `Api.Discovery`, `Api.Agents`, `Api.Activity` | 현황·설계 JSON 변환과 활동 기록 이벤트 해석 |
| `Page.Agents`, `Ui.AgentGraph` | 초안·설계안·진단·내보내기 화면과 인계 구조 SVG |
...
`App.Update.update`는 다음 상태와 `List Effect`를 반환합니다. 조회·저장·포커스 의도를 `LoadOrganizations`, `LoadWorkspace`, `SaveCommand`, `LoadDiscovery`, `SaveDiscovery`, `LoadAgents`, `SaveAgents`, `FocusElement`로 표현하고 `Main.perform`이 실행합니다. HTTP 오류의 표시 문자열 변환도 Main 경계에서 처리하므로 순수 전이는 브라우저 없이 검증할 수 있습니다.
...
`make test`는 Elm 테스트·포맷·타입 검사 후 Haskell 테스트를 실행합니다. `npm test`로 Elm 회귀 테스트만, `stack test`로 `test/`의 Haskell 테스트만 실행할 수 있습니다. Hspec·QuickCheck 기반의 도메인 검증과 함께 API 생명주기, 동시 데모 초기화, 조직 삭제·재생성, 다중 조직 격리, 실제 서버 시작 로직의 재시작과 저장 상태 복원을 검증합니다. `AgentSpec`은 도출 규칙, 진단 코드, 설계 저장 명령, 참조 필드 없는 과거 이벤트 읽기, 내보내기를, `AgentsSmokeSpec`은 HTTP 초안·저장·409/404/400·내보내기·재개방과 업무 참조 검증을 다룹니다.
...
서버 프로세스 관리에 POSIX 기능을 사용하므로 Linux/macOS 등 POSIX 환경이 필요합니다. 시작 테스트는 일회용 서버를 정지한 뒤 남을 수 있는 잠금 디렉터리를 정리하고 다음 시작을 검증합니다.
...
사이드바는 네 그룹입니다. 조직 운영(구성원, 목표, 책임, 권한), 운영과 개선(결과, 학습, 활동 기록), 조직 분석(조직 진단, 업무 흐름), 에이전트 설계(에이전트 초안, 에이전트 구조). 조직을 먼저 기록하고 운영 결과를 쌓은 뒤, 업무 흐름을 조사해 에이전트 구조로 모델링하는 순서입니다. 조직 목록과 조직 설정은 상단에서 엽니다.

- **조직 진단**: 분석 범위, 기준일, 확인된 사실·미확인·개선안 현황 항목과 근거.
- **업무 흐름**: 업무별 시작 조건, 입력, 도구, 산출물, 인계, 승인과 구성원·권한·업무 참조 연결.
- **구성원**: 이름, 역할, 보고 관계, 부서, 이메일, 재직 상태와 인계.
- **에이전트 초안**: 규칙 기반 역할 후보와 진단, 사람이 검토한 설계안 저장, 정의 파일 내보내기, 현황 검토 의견.
- **에이전트 구조**: 설계된 역할의 인계 구조와 사람 승인 지점.
- **활동 기록**: 조직별 감사 타임라인과 유형·기간 필터.
...
| GET/POST | `/api/organizations/:oid/discovery`, `/agents` | 조직 현황 문서와 에이전트 설계 (위 절 참고) |
| GET | `/api/organizations/:oid/agents/export` | 에이전트 정의 Markdown 첨부 파일 |
...
파일 저장은 프로세스 내 직렬화와 임시 파일 후 rename으로 처리하며 저장 성공 후에만 메모리 상태를 바꿉니다. 이벤트 파일이 손상되거나 순번이 어긋나면 시작을 거절합니다. 같은 파일을 여러 서버가 열지 못하도록 `.lock` 디렉터리를 사용합니다. 서버는 SIGINT와 SIGTERM을 받으면 저장소를 닫고 잠금을 해제한 뒤 종료합니다. SIGKILL 같은 강제 종료로 잠금이 남으면 해당 서버가 종료되었는지 확인한 뒤 잠금 디렉터리를 수동 정리해야 합니다. 파일 교체는 원자적이지만 전원 장애에 대한 fsync 보장은 제공하지 않습니다. 이벤트 전체를 재생하는 작은 조직용 MVP이며 대규모 저장소용 snapshot/compaction은 아직 없습니다.
...
[데모 워크스페이스](http://127.0.0.1:8081)를 엽니다. 최초 실행은 **6명·7개 목표**와 결과, 평가, 회고, 학습, 전략 기록, 그리고 현황 항목 3건과 참조 연결이 있는 **업무 흐름 4건**을 자동 생성합니다. 이 명령은 `MY_ORG_DEMO=1`을 지정하며 앱이 저장 위치를 `runs/demo/events.json`, 포트를 `8081`로 고정합니다. `MY_ORG_EVENT_FILE`·`MY_ORG_PORT`·`MY_ORG_SQLITE_FILE`의 기존 값을 사용하지 않습니다. 일반 `make run`의 `runs/local/events.json`과 분리됩니다.
...
| `Domain.Discovery`, `Domain.Agent`, `Domain.AgentDraft` | 현황 문서와 참조 필드, 에이전트 역할 타입, 규칙 기반 도출과 A001~A009 진단 |
| `Application.Command.Discovery`, `Command.Agents` | 참조 무결성을 포함한 현황 저장과 설계 저장 검증 |
| `Presentation.Agent`, `Serialization.Agent` | 진단 문구, Markdown 정의 파일 렌더링과 설계 JSON 코덱 |
...
5. **05 · 업무에서 에이전트 설계로**의 **이 단계 진행 →**를 누르면 에이전트 초안 화면의 가져오기 버튼으로 이동합니다. 문의 분류, 환불 검토, 제안서 작성, 계약 검토 네 후보를 설계안으로 가져오고 등급, 승인 주체, 인계 대상을 검토해 저장합니다. 저장하면 단계가 완료되고 에이전트 구조와 내보내기를 사용할 수 있습니다. 계약 검토 후보는 의도적으로 근거 미확인(A008)과 인계 미확인(A009)을 보여 줍니다.

가이드를 접거나 다시 펼칠 수 있으며 조직 안의 모든 화면에서 이어집니다. 버튼을 클릭했다는 이유만으로 완료 처리하지 않습니다. 새로고침해도 실제 데이터에서 진행 상태를 다시 계산합니다. 데모 조직에서만 가이드가 나타납니다. 그래프와 감사 기록도 안내 버튼으로 바로 이동할 수 있습니다.
...
데모 시드는 원래의 51개 시나리오 이벤트와 표식 뒤에 `DiscoverySaved` 한 건을 추가해 53개 이벤트를 만듭니다. `test/fixtures/`의 고정 이벤트 로그는 과거 52개 형식을 그대로 유지하며, 구형 데모 인식은 51개 시나리오 접두사만 비교하므로 기존 데모 저장소는 그대로 인식됩니다. 저장된 업무 흐름의 참조 필드와 `AgentRolesSaved` 이벤트는 새 기록에만 나타나고, 참조 필드가 없는 과거 업무 흐름은 빈 참조로 읽힙니다.
