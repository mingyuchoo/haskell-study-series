...
- **목표 대시보드 (Goal Dashboard)**: 조직·구성원·목표 생성, KPI, 상위 목표, 책임자 지정, 활성화, 결과·평가·전략 기록.
- **책임 지도 (Responsibility Map)**: 결과별 단일 최종 책임자, 필요 권한, 예산과 통제율. 사람·목표·지표·자원의 그래프 관계.
- **권한 지도 (Authority Map)**: 구성원의 결정 권한·예산 변경과 집중도. 집중도는 권한 종류 수 + 예산 보유 1점을 세는 규칙 기반 추정치입니다.
- **회고와 학습 (Review & Learning)**: 최신 결과와 평가를 함께 보존하는 회고, 학습, 결정 담당자·기한 및 감사 타임라인.
...
| `MyOrg.Server`, `static/src/` | JSON API, 네 화면, 실제 저장 상태로 판정하는 체험 가이드 |
...
초기 결과 세 개씩은 **가상 연속 측정 샘플**입니다. 과거에 실제 측정한 것처럼 감사 시각을 소급하지 않으며 모두 실제 가져온 시각을 기록합니다. 최신 샘플이 현재 실측값이 됩니다. Dashboard의 목표 상세에서 결과 추이를 볼 수 있습니다.

### 네 단계 따라 하기

1. 가이드의 **01 · 빈 책임 자리 채우기**의 **이 단계 진행 →**를 누릅니다. Responsibility Map의 파트너십 목표에서 `한유진 · 파트너십 책임자`를 지정합니다. 다시 안내를 따라 Dashboard의 해당 목표를 활성화합니다. 상태가 결과 대기로 바뀌면 첫 단계가 완료됩니다.
...
가이드를 접거나 다시 펼칠 수 있으며 네 화면 모두에서 이어집니다. 버튼을 클릭했다는 이유만으로 완료 처리하지 않습니다. 새로고침해도 실제 데이터에서 진행 상태를 다시 계산합니다. 데모 조직에서만 가이드가 나타납니다. 그래프와 감사 기록도 안내 버튼으로 바로 이동할 수 있습니다.
