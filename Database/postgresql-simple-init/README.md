...
- **설명**: 사용자 생성 (id가 이미 있으면 무시)
- **응답**: `created` 또는 `skipped`

### `PUT /users?id=1&name=Alice2`

- **설명**: 사용자 정보 수정
- **응답**: `updated` 또는 `not-updated`

### `DELETE /users?id=1`

- **설명**: 사용자 삭제
- **응답**: `deleted` 또는 `not-deleted`
...
2. **타입클래스 기반 추상화**: `UserRepository` 인터페이스를 통한 의존성 역전
